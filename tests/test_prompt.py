...
class CapturingClient:
    ...
    async def chat(self, messages, tools, *, system_prompt):  # noqa: ARG002
        ...
        self.user_messages.append(str(messages[-1].content))
...
def test_recall_rides_on_the_user_message_and_system_prompt_stays_stable(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    config = load_config(project_root=tmp_path)
    ...
    # A byte-identical system prompt across requests keeps the provider prefix cache valid.
    assert client.system_prompts[0] == client.system_prompts[1]
    assert "recalled-memory" not in client.system_prompts[1]
    assert "Current date" in client.user_messages[1]
    assert "recalled-memory" in client.user_messages[1]
    assert "uv run python -m pytest" in client.user_messages[1]
