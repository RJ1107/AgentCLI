...
def load_config(
    ...
    if root:
        project_config = _read_json(root / ".agentcli" / "config.json")
        if project_config:
...
def config_to_public_dict(config: AgentCliConfig) -> dict[str, Any]:
...
def _apply_env(data: dict[str, Any], env: dict[str, str | None]) -> dict[str, Any]:
    ...
    mappings: list[tuple[str, str, Any]] = [
        ...
        ("AGENTCLI_PROVIDER", "provider", str),
        ("AGENTCLI_MODEL", "model", str),
        ("AGENTCLI_BASE_URL", "base_url", str),
        ("AGENTCLI_CONTEXT_WINDOW", "context_window", int),
        ("AGENTCLI_MAX_TOKENS", "max_tokens", int),
        ("AGENTCLI_TEMPERATURE", "temperature", float),
    ]
    ...
    render_mode = env.get("AGENTCLI_RENDER_MODE") or env.get("AGENTCLI_RENDERER")
    if render_mode in {"plain", "inline"}:
        result["render_mode"] = render_mode

    if env.get("AGENTCLI_TUI") == "true":
        result["render_mode"] = "inline"

    for env_key, feature_key in [
        ("AGENTCLI_MCP", "mcp"),
        ("AGENTCLI_SKILL", "skill"),
        ("AGENTCLI_MEMORY", "memory"),
    ]:
        raw = env.get(env_key)
        if raw == "false":
...
def _config_to_dict(config: AgentCliConfig) -> dict[str, Any]:
